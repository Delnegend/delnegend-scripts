...
def get_args():
    ...
    parser.add_argument("-i", "--input", help="Input directory", default=os.getcwd(), type=str, required=False)
    parser.add_argument("-o", "--output", help="Output directory", default="./avif_output", type=str, required=False)
    parser.add_argument("-exit", help="Exit right after conversion", action="store_true", required=False)
    parser.add_argument("-no_success", help="Don't print success messages", action="store_true", required=False)
...
def convert(file: str, ext_: str, enc_: str, fallback_: str, repack_: str, log: str, detailed: bool):
    ...
    if execCmd(enc, log) != 0 and fallback_ == '':
        ...
        print(f'failed to encode') if detailed else None
        return file
    elif fallback_ != '':
...
def convert_batch(file_list: list, ext: str, enc: str, fallback: str, repack: str, stats: dict, log: str):
    if config['threads'] > 1:
        with ThreadPoolExecutor(max_workers=config['threads']) as executor:
            futures = {executor.submit(convert, file, ext, enc, fallback, repack, None, False): file for file in file_list}
        ...
        for file in file_list:
            ...
            if convert(file, ext, enc, fallback, repack, log, True) == 0:
                ...
                stats['original'] += original_size
                stats['converted'] += converted_size
                print(f'converted') if not args.no_success else None
            else:
                stats['fail'].append(file)
                print(f'{file} failed')
