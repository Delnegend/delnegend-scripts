...
def dim(img: str) -> tuple:
    raw = sp.check_output(
        f"ffprobe -v error -show_entries stream=width,height -of default=noprint_wrappers=1:nokey=1 {img}",
        stderr=sp.DEVNULL,
    )
...
def parse_args():
    ...
    parser.add_argument(
        "-format",
        help="Set output format, each seperated by a space, available: jpg, png (lossless), avif, jxl",
        type=str,
        default="jpg avif jxl",
    )
    parser.add_argument(
        "-model",
        help="Model of RealESRGAN (default: details)",
        type=str,
        default="details",
        choices=["details", "fast"],
    )
...
def main(args):
    ...
    if ((mode == "w") and (width < max_res)) or ((mode == "h") and (height < max_res)) or args.force:
        ...
        scale = (
            2
            if width * 2 >= max_res and height * 2 >= max_res
            else 3
            if width * 3 >= max_res and height * 3 >= max_res
            else 4
        )
        ...
        sp.call(
            f"realesrgan-ncnn-vulkan -n {model} -s {scale} -i {args.input} -o {upscaled_file} -f png",
            shell=True,
            stdout=sp.DEVNULL,
            stderr=sp.DEVNULL,
        )
        ...
        if mode == "w" and dim(upscaled_file)[0] > max_res:
            sp.call(
                f"ffmpeg -i {upscaled_file} -vf scale={max_res}:-1 {upscaled_file}_.png",
                shell=True,
                stdout=sp.DEVNULL,
                stderr=sp.DEVNULL,
            )
            ...
            sp.call(
                f"ffmpeg -i {upscaled_file} -vf scale=-1:{max_res} {upscaled_file}_.png",
                shell=True,
                stdout=sp.DEVNULL,
                stderr=sp.DEVNULL,
            )
    ...
    for fmt in out_formats:
        ...
        encode(
            upscaled_file if os.path.exists(upscaled_file) else args.input,
            output + "." + fmt,
        )
...
if __name__ == "__main__":
    ...
    print(
        "For more efficient jpg encoding (mozjpeg), consider adding cjpeg-static to PATH. Ignore this if you don't encode to jpg."
    ) if not inPath("cjpeg-static") else None
